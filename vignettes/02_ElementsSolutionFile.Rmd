...
Various customisations can be done using css. This way it is also possible to design different types of highlight blocks in the same problem set (i.e. for simple Highlights, Tips and Dangers). See the [example problem set]() (and the corresponding [source file](https://github.com/LeverageData/RTutor/blob/master/inst/examples/HighlightExample_sol.Rmd)) for a more detailed guide on how to use highlight blocks.
